...
class SimulationEngine:
    ...
    def _save_rich_analysis_artifacts(self, run_id: UUID, result, agent_id: int, initial_cash: int) -> None:
        """Generate and persist rich-analysis artifacts (Step 8).

        Saves PnL chart, inventory chart, fill scatter, and the full
        analysis bundle JSON for a given agent.
        """
        try:
            bundle = self.analyzer.compute_rich_analysis(result, agent_id, initial_cash)
        except Exception:
            logger.warning("Rich analysis computation failed for run %s", run_id, exc_info=True)
            return
        ...
        try:
            self.repo.save_artifact(
                run_id=run_id,
                artifact_type=ArtifactType.RICH_ANALYSIS,
                path="rich_analysis.json",
                content=bundle.model_dump_json().encode(),
            )
        except Exception:
            logger.warning("Failed to save rich analysis JSON for run %s", run_id, exc_info=True)

        # Save charts
        chart_defs: list[tuple[str, ArtifactType, object]] = [
            ("pnl_curve.png", ArtifactType.PNL_CHART, bundle.pnl_curve),
            ("inventory.png", ArtifactType.INVENTORY_CHART, bundle.inventory_trajectory),
            ("fill_scatter.png", ArtifactType.FILL_SCATTER, bundle.fills),
        ]
        chart_methods = {
            "pnl_curve.png": self.analyzer.plot_pnl_curve,
            "inventory.png": self.analyzer.plot_inventory,
            "fill_scatter.png": self.analyzer.plot_fills_vs_mid,
        }
        for filename, art_type, data in chart_defs:
            try:
                plot_fn = chart_methods[filename]
                fig = plot_fn(data)  # type: ignore[arg-type]
                content = AnalysisService.figure_to_bytes(fig)
                self.repo.save_artifact(
                    run_id=run_id,
                ...
                logger.warning("Failed to save rich chart %s for run %s", filename, run_id, exc_info=True)
