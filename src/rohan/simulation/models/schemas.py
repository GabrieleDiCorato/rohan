...
class OrderBookL1Schema(pa.DataFrameModel):
    ...
    time: Series[int] = pa.Field(ge=0, description="Nanoseconds since midnight")
    bid_price: Series[float] = pa.Field(nullable=True, ge=0, description="Best bid price in cents (float64 for NaN compat)")
    bid_qty: Series[float] = pa.Field(nullable=True, ge=0, description="Quantity at best bid")
    ask_price: Series[float] = pa.Field(nullable=True, ge=0, description="Best ask price in cents (float64 for NaN compat)")
    ask_qty: Series[float] = pa.Field(nullable=True, ge=0, description="Quantity at best ask")
...
class OrderBookL2Schema(pa.DataFrameModel):
    ...
    price: Series[float] = pa.Field(ge=0, description="Price at level in cents (float64 for NaN compat)")
    qty: Series[float] = pa.Field(ge=0, description="Quantity at level")
    timestamp: Series[pa.DateTime] = pa.Field(description="Wall-clock timestamp")
